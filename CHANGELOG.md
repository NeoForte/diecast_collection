...
## 6.3.8
## 6.4.0
- Fixed the reversed top header layout so the Pocket 64 banner stays fully clear of the profile icon on iPhone Safari.
- Banner remains left, profile remains right; nothing below the header was changed.
...
- Replaced the stack of version-specific packaging YAML files with one reusable `.github/workflows/package-release.yml` workflow that reads the release number from `version.json`.
- Folded the standalone v6.3.1 CSS-audit summary into `ARCHITECTURE.md` and removed the extra audit file.
- No application behavior, Safari viewer behavior, Sets, backup/restore logic, data model, or visual layout was intentionally changed.

## Pocket 64 v6.3.2 — Header CSS Surgical Cleanup


- First CSS consolidation pass, limited to the main header/banner chain.
- Removed five byte-equivalent historical header alignment rule blocks from the v2.4.7 section.
- The identical later declarations remain in the v2.5.2/v2.6.x rules, preserving the final cascade and approved visual layout.
- No banner sizing, positioning, profile icon, navigation, Sets, viewer, backup, or data behavior was intentionally changed.
- viewer-core.js remains untouched and active.

## Pocket 64 v6.3.1 — CSS Audit / Repository Hygiene


- Audited the 5,710-line styles.css for duplicate selectors and legacy override chains.
- No CSS declarations were removed or reordered in this release; visual behavior should remain unchanged.
- Added CSS-AUDIT-v6.3.1.md documenting duplicate-density areas and safe cleanup candidates.
- Removed retired p64-v525-patch.js, set-flow.js, and set-ui.js files that were still physically present in the Git snapshot despite no longer being runtime-loaded.
- Kept viewer-core.js unchanged as the known-good Safari/iPhone viewer dependency.
- Version/cache references bumped to 6.3.1.

## Pocket 64 v6.3.0


Source-hardening release.

- Moved retired editor-field visibility from runtime JavaScript into source markup.
- Kept the retired Series/Collection, Category, and Series/Collection Number controls in the DOM for backward data compatibility, but they no longer require editor-core.js to hide/reflow them on every page show.
- General Number is source-defined as the full-width visible number field.
- Preserved the known-good v6.2.10 viewer-core.js photo viewer unchanged in behavior.
- Corrected ARCHITECTURE.md to document viewer-core.js as an active compatibility dependency after the v6.2.9 regression.
- No intentional changes to car data, Sets, backup/restore, Supabase, camera behavior, or photo-viewer gestures.

## Pocket 64 v6.2.10


Viewer source consolidation / hardening

- Retired viewer-core.js.
- Moved the proven full-screen viewer sizing into permanent styles.css rules.
- Moved the Photos-like pinch-anchor/pan behavior into app.js, where the viewer already lives.
- Removed the old version-stamped viewer style IDs, install markers, and MutationObserver cleanup layer.
- Removed inline image-size overrides that viewer-core had to repeatedly undo.
- Preserved swipe navigation at 1x, pinch zoom up to 4x, zoomed panning, close behavior, and single-photo counter hiding.
- No Set, backup/restore, camera, Supabase schema, or collection-data behavior changed.

## Pocket 64 v6.2.8


Legacy support-layer hardening pass.

- Removed showcase-sync.js global applyPatch retry timers.
- Removed its main-view MutationObserver that repeatedly reran the entire support layer.
- Support/UI helpers now initialize once and keep only feature-specific observers where needed.
- Renamed the Set modal patch helper to a neutral permanent enhancement name.
- Migrated restore-finalization session state to a neutral key while still accepting the old v5.0.9 key for in-flight compatibility.
- Removed stale v5.0.9/v5.1.1 ownership/error labels from active backup/restore code.
- Updated the stale service-worker registration cache key from v5.0.0 to v6.2.8.
- No visible feature, Set schema, backup format, photo, or collection-data changes intended.

## Pocket 64 v6.2.7


Hardening / source cleanup release. No intended visible behavior changes.

- Removed the transitional p64-v525-patch.js bootstrap.
- Permanent editor, camera-safety, viewer, and Set-visual modules now load directly from index.html.
- Moved the bootstrap's final live Safari/PWA retirement behavior into app.js.
- Preserved the private-photo cache while retiring obsolete service-worker/app caches.
- Removed stale set-flow.js and set-ui.js files that were already superseded by set-core.js in v6.2.6.
- Unified primary APP_VERSION ownership at 6.2.7 and updated deployment cache keys.
- Preserved pocket64-empty-slot.jpg and its active Set empty-slot CSS dependency.

## Pocket 64 v6.2.6 — Set Core Consolidation


- Merged the proven set-flow.js and set-ui.js behavior into one permanent set-core.js module.
- Removed the retired set-flow.js and set-ui.js helper files.
- Replaced old v5.3.8/v6.0.9-style Set DOM IDs/classes with permanent neutral names.
- Loads set-core.js directly from index.html instead of through the transitional p64-v525 bootstrap.
- Preserves current Set creation, Add to Set, official/reference Set materialization, position assignment, and local/cloud state behavior.
- Preserves the restored empty Set slot artwork and bumps its cache key to v6.2.6.
- Leaves p64-v525-patch.js in place for the remaining editor/camera/viewer/set-visual compatibility bootstrap, ready for a later removal pass.

## Pocket 64 v6.2.5 — Set Empty-Slot Cache Hotfix


- Fixes the restored Set empty-slot artwork not appearing in Safari after v6.2.4.
- The image asset is present and the Set renderer still applies the correct `set-empty-photo` class.
- Bumps the stylesheet cache key and the empty-slot image URL so Safari/edge caches cannot keep serving the earlier missing-asset response.
- No collection, Set, photo, database, Supabase schema, or backup-format changes.

Test focus: open any incomplete Set and verify every EMPTY SLOT card shows the original Pocket 64 missing-card artwork.

---

## Legacy release history from the previous README
...
- Bumped PWA cache/assets to 3.0.6.


## v3.0.7
- Added Settings → Clear Collection for clean backup/restore testing.
...
## v3.1.1
- Newest sorting now uses the most recent add/update activity timestamp, so adding 1 to a selected duplicate moves that car to the top without rewriting its original created_at date.
- Added a dedicated one-tap × button inside Home search to instantly clear the query and return to the full collection.


## v3.1.2
- Removed the camera photo framing/crop step, including pinch-to-zoom and drag positioning.
